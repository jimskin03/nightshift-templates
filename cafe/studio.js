(() => {
  ...
  const loadSite = async siteId => {
    if (!siteId) { currentSite = null; setEditorEnabled(false); return; }
    try { currentSite = await adapter.load(siteId); config = { ...structuredClone(manifest.defaults), ...(currentSite.draft_data || {}) }; $('site-title').textContent = currentSite.name; renderForm(); setEditorEnabled(true); showError(''); announce(`Loaded ${currentSite.name}. Draft belongs to your authenticated account.`); send(); }
    catch (error) { currentSite = null; setEditorEnabled(false); announce(`Draft load failed: ${apiMessage(error)}`); showError(apiMessage(error)); }
  };
  ...
  window.NightshiftCafeManifest.then(async loaded => {
    ...
    try { adapter = window.NightshiftAuthAdapter.create(); user = await adapter.getSessionUser(); if (!user) { $('auth-required').hidden = false; announce('Sign in required to load your sites.'); return; } $('auth-user').hidden = false; $('auth-user').textContent = `Signed in as ${user.email || 'CryptGreg user'}`; $('auth-required').hidden = true; await loadGallery(); }
